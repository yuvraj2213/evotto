...
const AdminPendingVehicles = () => {
  ...
  const handleAcceptClick = async (e, id, name, image, description) => {
    e.preventDefault();
    setLoading(true);
    ...
    try {
      // Step 1: Add to Rental Vehicles Database
      const addResponse = await fetch(
        `${baseURL}/api/admin/acceptPendingVehicle`,
        {
          method: "POST",
          headers: {
      ...
      if (addResponse.ok) {
        ...
        const deleteResponse = await fetch(
          `${baseURL}/api/admin/deletePendingVehicle/${id}`,
          {
            method: "DELETE",
            headers: {
              Authorization: authorizationToken,
        ...
        if (deleteResponse.ok) {
          toast.success(
            "Vehicle moved to Rental Vehicles and removed from Pending."
          );
